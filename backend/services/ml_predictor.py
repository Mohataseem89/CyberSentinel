...
class MLPredictor:
    ...
    def load_model(self) -> bool:
        ...
        calibrator_path = self.data_dir / "probability_calibrator.pkl"
        manifest_path = self.data_dir / "model_manifest.json"
        try:
            if not model_path.exists() or not calibrator_path.exists() or not manifest_path.exists():
                raise FileNotFoundError("A compatible model, probability calibrator, and model_manifest.json are required.")
            ...
            if manifest.get("calibration", {}).get("method") != "isotonic":
                raise ValueError("Model manifest does not declare the required probability calibration.")
            ...
            if not hasattr(calibrator, "transform"):
                raise ValueError("Model probability calibrator is invalid.")
            self.model, self.calibrator, self.manifest, self.unavailable_reason = model, calibrator, manifest, ""
            logger.info("Validated ML model loaded: %s", manifest.get("model_version", "unknown"))
            return True
        except Exception as error:
            self.model, self.calibrator, self.manifest, self.unavailable_reason = None, None, None, str(error)
    ...
    def predict(self, url: str) -> dict:
        ...
        try:
            ...
            raw_probabilities = dict(zip(self.model.classes_, self.model.predict_proba(frame)[0], strict=True))
            phishing_probability = float(self.calibrator.transform([raw_probabilities["phishing"]])[0])
            legitimate_probability = float(1 - phishing_probability)
            ...
            return {
                ...
                "message": "Calibrated ML probability is one risk signal, not a safety guarantee.",
            }
        except Exception:
